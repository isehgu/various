<?php
	...
	function f_tableDisplay($type)
  {
    ...
    if($type == 'test')
    {
      $sql_query = "select test_id as id,test_name as name,description from test_case where active_status = 1 order by test_name";
    }
    else
    {
      $sql_query = "select suite_id as id,suite_name as name,description from test_suite where active_status = 1 order by suite_name";
    }
    $result = $db->query($sql_query) or die($db->error);
 ...
 */
  ...
  //Input:	none
	//Output:	echo out the html needed to display env autoupgrade setting and versions
	function f_tableEnvAutoupgrade()
	{
		global $db;

    //$upgrade_array = array(0=>'No Release Detected',1=>'Upgrade Started',2=>'Upgrade Completed');
    $autoupgrade_array = array(0=>'Disabled',1=>'Enabled');
    
		$sql_query = "select * from env_upgrade order by env_id,app_name";
		$result = $db->query($sql_query) or die($db->error);
    while($row = $result->fetch_assoc())
		{
			$euid='';
      ...
      $target_status='';
			...
			$env = f_getEnvFromId($row['env_id']);
      $euid = $row['env_upgrade_id'];
      $app = $row['app_name'];
      ...
      if($autoupgrade == 0)
      {
        $autoupgrade_status = 1;
        $autoupgrade_btn_class = 'btn btn-warning';
      }
      else
      {
        $autoupgrade_status = 0;
        $autoupgrade_btn_class = 'btn btn-primary';
      }
      
      if(!$version) //If there's no version, thus no release
      {
        $upgrade_status = 0;
        $upgrade_btn_class = 'btn disabled';
        $upgrade_btn = 'No Release Detected';
      }
      else //Now the version exists, thus new release detected
      {
        if($upgrade_status == 0)
        {
					if($autoupgrade == 0) //if autoupgrade is disabled, then shouldn't allow upgrade
					{
						$upgrade_status = 1;
						$upgrade_btn_class = 'btn btn-primary disabled';
						$upgrade_btn = 'Start Upgrade';
					}
					else
					{
						$upgrade_status = 1;
						$upgrade_btn_class = 'btn btn-primary';
						$upgrade_btn = 'Start Upgrade';
					}
        }
        elseif($upgrade_status == 1) //if upgrade already started, move it to upgrade completed
        {
          $upgrade_status = 2;
          $upgrade_btn_class = 'btn btn-success';
          $upgrade_btn = 'Complete Upgrade';
        }
        else //if upgrade is completed, do nothing and disable btn
        {
          $upgrade_status = 2;
          $upgrade_btn_class = 'btn btn-success disabled';
          $upgrade_btn = 'Upgrade Completed';
        }
      }
			...
			echo "<tr>
							...
							<td>$version</td>
              <td>
                <form class='autoupgrade_form' action='env_autoupgrade.php' method='get'>
                  <input type='hidden' name='env_upgrade_id' value='$euid'>
                  <input class='autoupgrade_status_input' type='hidden' name='status' value='$autoupgrade_status'>
                  <button type='submit' class='$autoupgrade_btn_class autoupgrade_btn'>$autoupgrade_array[$autoupgrade]</button>
                </form>
              </td>
							<td>
                <form class='upgrade_form' action='env_upgrade.php' method='get'>
                  <input type='hidden' name='env_upgrade_id' value='$euid'>
                  <input class='upgrade_input' type='hidden' name='status' value='$upgrade_status'>
                  <button type='submit' class='$upgrade_btn_class upgrade_btn'>$upgrade_btn</button>
                </form>
              </td> 
						</tr>";
		}//End of while
	}//End of f_tableEnvAutoupgrade
	...
	function f_statCount($type)
	{
		...
		elseif($type == 'test') $sql_query = "select count(*) as count from test_case where active_status = 1 ";
  ...
  //Get env name from env_id
	function f_getEnvFromId($eid)
	{
		global $db;
		$sql_query = "select env_name as name from test_env where env_id = $eid limit 1";
		$result = $db->query($sql_query) or die($db->error);
		$row = $result->fetch_assoc();
		return $row['name'];
	}
