<?php
	...
	function f_displayAll()
	{
		...
		if($_GET['textstring'])
		{
			$sql_query = "SELECT iseappname,instancename,instanceversion,timestamp,instanceNode,type 
						  FROM register ".f_searchBuilder()." 
						  ORDER BY timestamp desc, instanceversion desc, iseappname asc, instancename asc, instancenode asc, type asc";
		}
		# ELSE IF 'date' is selected
		elseif($_GET['datestring'])
		{
			$sql_query = "SELECT iseappname,instancename,max(instanceversion) as instanceversion,max(timestamp) as timestamp,instanceNode,type 
						  FROM register 
						  WHERE timestamp <= '".$_GET['datestring']."' 
						  GROUP BY iseappname,instancename, instanceversion, instanceNode,type 
						  ORDER BY timestamp desc,iseappname asc,instancename asc,instancenode asc, type asc";
		}
		...
		else
		{
			$sql_query = "SELECT iseappname, instancename, max(instanceversion) as instanceversion, max(timestamp) as timestamp, instanceNode ,type 
						  FROM register 
						  GROUP BY iseappname, instancename, instanceNode, type
						  ORDER BY timestamp desc, iseappname asc, instancename asc, instancenode asc, type asc";
		}
